...
function Step2() {
  const [data, isValid, isSubmitting, submitError] = useUnit([$data, $step2Valid, $isSubmitting, $submitError]);
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            value={data.password}
            onChange={(e) => fieldChanged({ field: "password", value: e.target.value })}
            placeholder="Не менее 6 символов"
            autoComplete="new-password"
            minLength={6}
            required
            className="w-full px-4 py-3 pr-12 rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary text-sm transition-colors"
          />
          <button
            type="button"
  ...
  return (
    <div className="px-8 pb-10 text-center flex flex-col items-center">
      ...
      <p className="text-muted-foreground mb-8 max-w-sm">
        Аккаунт создан для <span className="font-medium text-foreground">{data.email}</span>.
        Перейдите в личный кабинет, чтобы заполнить профиль и добавить прайс-лист.
      </p>
